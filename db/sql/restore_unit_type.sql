-- Restore the system (company_id = NULL) unit_type lookup options that were
-- deleted from the DB. Idempotent + additive: each row is inserted only when
-- it isn't already present — existing rows are never modified or removed.
